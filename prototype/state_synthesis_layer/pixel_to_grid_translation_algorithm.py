# First, need some way to bring the data in. Kafka big unfilled hole

# from data_transform import screen_data_capture
import numpy as np
import pandas as pd

def create_pixel_grid(screen_data_capture):
    # print(screen_data_capture)
    """
    ...
    # print('len scanline_rows pre trim: ', len(scanline_rows))
    """
    ...
    print('len scanline_rows post trim: ', len(scanline_rows))
    try:
        ...
        print('index error')
        # print(f'x up range {x_up_range} | y up range {y_up_range} | x breaks {x_breaks} | y breaks {y_breaks}')
...
def reduce_pixel_square(target):
    column_counts = [np.argmax(np.bincount(np.array(row))) for row in target if len(row) > 0]
    row_count = np.bincount(np.array(column_counts))
    return np.argmax(row_count)
